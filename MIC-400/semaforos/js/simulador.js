...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function limitarNumero(valor, minimo, maximo, defecto) {
    ...
    return Math.min(
      maximo,
      Math.max(minimo, Math.round(numero))
    );
  }
  ...
  function ejecutarFase(nuevoIndice) {
    ...
    indiceFase =
      ((nuevoIndice % fases.length) + fases.length) %
      fases.length;
  ...
  function establecerEstadoEjecucion(activo, texto) {
    ...
    elementos.textoEjecucion.textContent =
      texto || (activo ? "En ejecución" : "Detenido");

    elementos.puntoEjecucion.classList.toggle(
      "estado-ejecucion__punto--activo",
      activo
    );
  }
  ...
  function porcentaje(restante, total) {
    ...
    return Math.max(
      0,
      Math.min(100, (restante / total) * 100)
    );
  }
  ...
  function actualizarPanel(
    ...
    barra.style.width =
      porcentaje(datos.restante, datos.total) + "%";
  }
});
